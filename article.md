...
C言語やC++は、その複雑な文法から、**学習書に間違いが書かれている**ことが珍しくありません。  
そのため、冗談抜きで**「迷ったら規格書を読め」**と勧められることがままあります。  
[規格準拠度が高いコンパイラ](https://ja.wikipedia.org/wiki/Clang)の警告を読めばヘーキヘーキ……[と言うわけには行かない](http://boleros.hateblo.jp/entry/2015/05/18/171520)のが現実ですので、日々精進しましょう。
...
近年のコンパイラは優秀ですので、手動でガリガリとアセンブラを書くよりずっと高速になることが多いです。  
...
結論から言いますと、C++はC言語にオブジェクト指向を拡張したもの**ではありません**。  
もちろんオブジェクト指向要素もあるのですが、それ以外にも[template](http://qiita.com/narumi_/items/f656678c78d50c40bc1c)など様々な機能が拡張されています。  
また、C言語もC言語で**独自の発展**を遂げましたので、C++の下位互換ではなくなっています。
